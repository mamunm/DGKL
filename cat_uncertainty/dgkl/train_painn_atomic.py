"""Script to train a PaiNN model on a dataset using DGKL."""
import os
import time
import torch
import gpytorch
...
DATA_DICT = {
    ...
    "cathub_atomic": "catalysis_hub/N_free"
}
LMDB_FILES = {
    "cathub": "CatHub.lmdb",
    "eagle": "eagle_data.lmdb",
    "cathub_atomic": "N_free_data.lmdb"
}
...
def train_painn(
    ...
    painn_params = PaiNNModelParams(
        ...
        num_radial=config.get("num_radial", 20),
        cutoff=10,
        max_neighbors=32,
        n_atom_type=100,
    ...
    feature_extractor = PaiNN(config=painn_params)
    if config.get("layer_norm", False):
        feature_extractor.output_network.append(
            torch.nn.LayerNorm(
